...
public class SaveLoad {
	...
	public static void load(int[] playerData, boolean[] isDone, String[] name, int[] safeCombo, Scanner input) {
		try (Scanner fin = new Scanner(new File(name[3] + ".txt"))) {
			...
			case 1:
				...
				break;
			default:
				Tools.slowText("Error: something is not right.");
				System.exit(0);
			}
			...
			while (isGood == false) {
				...
				case "y":
					...
					save(playerData, isDone, name, safeCombo, 1);
	...
	public static void save(int[] playerData, boolean[] isDone, String[] name, int[] safeCombo, int place) {
		...
		try (PrintWriter fout = new PrintWriter(new File(name[3] + ".txt"))) {
